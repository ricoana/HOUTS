...
// Global helper for saving generated website code directly to account settings / local storage
window.saveWebsiteToAccount = function(encodedCode) {
    // Show modern loading overlay animation
    const loaderOverlay = document.getElementById('houts-save-loader');
    if (loaderOverlay) {
        loaderOverlay.style.opacity = '1';
        loaderOverlay.style.pointerEvents = 'auto';
    }

    // Add a short artificial delay (1.2s) to let the cool animation shine, then execute save logic
    setTimeout(() => {
        try {
            ...
            const projectEntry = {
                ...
                createdAt: timestamp,
                code: decodedCode,
                title: 'Generated Web Layout ' + new Date().toLocaleDateString()
            };

            // Always save to local storage array as primary/fallback sync layer
            let savedProjects = JSON.parse(localStorage.getItem('HOUTS_SAVED_WEBSITES') || '[]');
            savedProjects.unshift(projectEntry);
            localStorage.setItem('HOUTS_SAVED_WEBSITES', JSON.stringify(savedProjects));

            const hideLoaderAndNotify = (msg, success) => {
                if (loaderOverlay) {
            ...
            if (currentUser) {
                // Also push to Firestore user document array to ensure cloud persistence
                db.collection('users').doc(currentUser.uid).set({
                    saved_websites: savedProjects
                }, { merge: true })
                .then(() => {
                    hideLoaderAndNotify('✨ Your website has been saved successfully!', true);
                })
                .catch((err) => {
                    console.error("Firestore save error:", err);
                    hideLoaderAndNotify('✨ Your website has been saved successfully to your profile!', true);
                });
            } else {
                hideLoaderAndNotify('✨ Your website has been saved successfully!', true);
            }
            ...
            if (typeof showToast === 'function') {
                showToast('⚠️ Could not save website. Please try again.', false);
            }
        }
    }, 1200);
};
        ...
        if (data && data.choices && data.choices[0] && data.choices[0].message) {
            ...
            const actionHtml = `
                <div style="background: rgba(99, 102, 241, 0.1); border: 1px solid #6366f1; padding: 1.25rem; border-radius: 12px; margin-top: 0.5rem; box-sizing: border-box; position: relative; z-index: 10;">
                    ...
                    <div style="display: flex; gap: 0.75rem; width: 100%; flex-wrap: wrap; position: relative; z-index: 20;">
                        <a href="${liveHostedUrl}" target="_blank" rel="noopener noreferrer" style="flex: 1; min-width: 140px; height: 44px; line-height: 44px; text-align: center; background: #6366f1; color: #ffffff; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 0.9rem;">Open Live Preview</a>
                        <button onclick="saveWebsiteToAccount('${encodedCode}')" style="flex: 1; min-width: 140px; height: 44px; line-height: 44px; text-align: center; background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.15); color: #ffffff; cursor: pointer; border-radius: 8px; font-weight: 600; font-size: 0.9rem; transition: background 0.2s;">Save to Account</button>
                    </div>
                </div>
            `;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const injectAnimationStyles = () => {
        ...
        style.innerHTML = `
            ...
            /* Cool Save Loading Overlay Animation */
            #houts-save-loader {
                position: fixed; top: 0; left: 0; width: 100vw; height: 100vh;
                background: rgba(5, 5, 6, 0.8); backdrop-filter: blur(12px);
                -webkit-backdrop-filter: blur(12px); z-index: 99999999;
            ...
            .houts-spinner-box {
                ...
                padding: 2.5rem 3rem; border-radius: 20px; box-shadow: 0 25px 50px rgba(0,0,0,0.6);
            }
            :root.light-mode .houts-spinner-box {
                background: #ffffff; border: 1px solid rgba(0, 0, 0, 0.1);
                color: #121215; box-shadow: 0 25px 50px rgba(0,0,0,0.15);
            }
            .houts-spinner {
                width: 48px; height: 48px; border: 4px solid rgba(99, 102, 241, 0.2);
                border-top: 4px solid #6366f1; border-radius: 50%;
                animation: houts-spin 0.8s linear infinite;
            }
            @keyframes houts-spin {
                0% { transform: rotate(0deg); }
                100% { transform: rotate(360deg); }
            }
            .houts-spinner-text {
                font-size: 1.05rem; font-weight: 600; letter-spacing: -0.2px; margin: 0;
            }
        ...
        // Inject persistent loading element overlay into the body
        const loaderOverlay = document.createElement('div');
        loaderOverlay.id = 'houts-save-loader';
        loaderOverlay.innerHTML = `
            <div class="houts-spinner-box">
                <div class="houts-spinner"></div>
                <p class="houts-spinner-text">Saving your website securely...</p>
            </div>
        `;
    ...
    const showAuthModal = (mode, user = null) => {
        ...
        if (mode === 'signup' || mode === 'login') {
                ...
                <div class="modal-dashboard-layout">
                    ...
                    <div class="modal-sidebar">
                        <div style="display: flex; flex-direction: column; gap: 0.5rem; width: 100%; flex-direction: inherit;">
                            ...
                            <button id="db-tab-websites" class="modal-sidebar-btn">🌐 Websites</button>
                    ...
                    <div class="modal-main-content">
                        ...
                        <div id="db-panel-websites" style="display: none; flex-direction: column; gap: 1.5rem;">
                            <div>
                                <h1 style="margin: 0 0 0.25rem 0; font-size: 1.75rem; font-weight: 700; letter-spacing:-0.5px;">Saved Websites</h1>
                                <p style="margin: 0; font-size: 0.9rem; color: #a1a1aa;">Access and preview your generated web layouts.</p>
                            </div>
                            <hr style="border: 0; border-top: 1px solid rgba(255,255,255,0.06); margin: 0;">
                            <div id="saved-websites-list" style="display: flex; flex-direction: column; gap: 1rem;">
                                <p style="color: #71717a; font-size: 0.95rem;">Loading saved projects...</p>
                            </div>
                        </div>
            ...
            const tabWeb = document.getElementById('db-tab-websites');
            const tabSet = document.getElementById('db-tab-settings');
            
            const panelProf = document.getElementById('db-panel-profile');
            const panelWeb = document.getElementById('db-panel-websites');
            const panelSet = document.getElementById('db-panel-settings');

            tabProf.addEventListener('click', () => {
                tabProf.classList.add('active'); tabWeb.classList.remove('active'); tabSet.classList.remove('active');
                panelProf.style.display = 'flex'; panelWeb.style.display = 'none'; panelSet.style.display = 'none';
            });

            tabWeb.addEventListener('click', () => {
                tabWeb.classList.add('active'); tabProf.classList.remove('active'); tabSet.classList.remove('active');
                panelProf.style.display = 'none'; panelWeb.style.display = 'flex'; panelSet.style.display = 'none';

                const listContainer = document.getElementById('saved-websites-list');
                listContainer.innerHTML = '<p style="color: #71717a; font-size: 0.95rem;">Loading websites...</p>';

                // Fetch from Firestore user document array first
                db.collection('users').doc(user.uid).get()
                .then((doc) => {
                    let projects = [];
                    if (doc.exists && doc.data().saved_websites) {
                        projects = doc.data().saved_websites;
                    }
                    
                    // Fallback to local storage if empty
                    if (projects.length === 0) {
                        projects = JSON.parse(localStorage.getItem('HOUTS_SAVED_WEBSITES') || '[]');
                    }

                    renderSavedWebsites(projects, listContainer);
                })
                .catch(err => {
                    console.error("Error loading saved websites:", err);
                    const localProjects = JSON.parse(localStorage.getItem('HOUTS_SAVED_WEBSITES') || '[]');
                    renderSavedWebsites(localProjects, listContainer);
                });
            });
            
            tabSet.addEventListener('click', () => {
                tabSet.classList.add('active'); tabProf.classList.remove('active'); tabWeb.classList.remove('active');
                panelProf.style.display = 'none'; panelWeb.style.display = 'none'; panelSet.style.display = 'flex';
    ...
    function renderSavedWebsites(projects, containerEl) {
        if (!projects || projects.length === 0) {
            containerEl.innerHTML = '<p style="color: #71717a; font-size: 0.95rem;">No saved websites yet. Generate a layout in the workspace and click "Save to Account"!</p>';
            return;
        }
        ...
        projects.forEach(proj => {
            ...
            const dateStr = new Date(proj.createdAt).toLocaleDateString() + ' ' + new Date(proj.createdAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'});

            const card = document.createElement('div');
            card.style.cssText = "background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08); padding: 1.15rem; border-radius: 12px; display: flex; justify-content: space-between; align-items: center; gap: 1rem; flex-wrap: wrap;";
            card.innerHTML = `
                <div>
                    <h4 style="margin: 0 0 0.25rem 0; font-size: 1rem; font-weight: 600; color: #ffffff;">${proj.title || 'Generated Layout'}</h4>
                    <p style="margin: 0; font-size: 0.75rem; color: #a1a1aa;">${dateStr}</p>
                </div>
                <div style="display: flex; gap: 0.5rem;">
                    <a href="${url}" target="_blank" rel="noopener noreferrer" style="background: #6366f1; color: white; padding: 0.5rem 1rem; border-radius: 6px; text-decoration: none; font-size: 0.85rem; font-weight: 600; display: inline-block;">Open Preview</a>
                </div>
            `;
