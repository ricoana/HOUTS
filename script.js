...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (menuToggle && navMenu) {
        ...
        // Close nav when hitting text links
    ...
    const injectModalHTML = () => {
        ...
        styleTag.innerHTML = `
            #auth-modal-overlay {
                ...
                background: rgba(5, 5, 6, 0.9); backdrop-filter: blur(24px);
                    ...
                    <div class="modal-main-content">
                        <div id="db-panel-profile" style="display: flex; flex-direction: column; gap: 1.5rem;">
                            ...
                            <div>
                                ...
                                <span id="db-username-text" style="font-size: 1.2rem; font-weight: 600; color: #ffffff;">Loading...</span>
                            </div>
                            <div>
            ...
            document.getElementById('db-username-form').addEventListener('submit', (e) => {
                ...
                if (newName) {
                    ...
                    .then(() => {
                        ...
                        saveBtn.textContent = 'Saved! ✓'; saveBtn.style.background = '#22c55e';
                        setTimeout(() => { saveBtn.textContent = 'Save'; saveBtn.style.background = '#6366f1'; }, 2000);
                    });
                }
            });
        }
    ...
    // --- 3. DYNAMIC BAR CONFIGURATION & DISCOVERY ---
    const navMenuContainer = document.querySelector('.nav-menu');
    const launchProjectBtn = document.getElementById('hero-launch-project-btn');
    
    let loginBtn = document.querySelector('.auth-link-login');
    let signupBtn = document.querySelector('.nav-cta-btn');
    ...
    auth.onAuthStateChanged((user) => {
        ...
        if (user) {
            // Hide standard anonymous tags
            if (loginBtn) loginBtn.style.display = 'none';
            if (signupBtn) signupBtn.style.display = 'none';

            // Show Launch button ONLY if authenticated
            if (launchProjectBtn) {
                launchProjectBtn.style.display = 'inline-block';
            }
            ...
            if (navMenuContainer) {
                ...
                // Insert gracefully before Login link position
                navMenuContainer.insertBefore(settingsBtn, navMenuContainer.querySelector('.auth-link-login'));
            ...
            // Explicitly hide the Launch Project button if logged out
            if (launchProjectBtn) {
                launchProjectBtn.style.display = 'none';
            }
        }
    });
    ...
    if (launchProjectBtn) {
        launchProjectBtn.addEventListener('click', (e) => {
            e.preventDefault();
            alert('Initialization vector active! Processing project space...');
        });
    }
});
