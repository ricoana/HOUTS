...
// --- 2. LOCAL ENVIRONMENT MANAGEMENT ENGINE (GROQ) ---
let activeGroqApiKey = null;
let chatHistoryArray = []; // Tracks chat history for conversational flow

function initializeLocalGroq() {
    const cachedKey = localStorage.getItem('HOUTS_SECURE_GROQ_KEY');
    if (!cachedKey || cachedKey.trim() === "") {
        activeGroqApiKey = null;
        chatHistoryArray = [];
        return false;
    }

    // Sanitize the key string cleanly
    activeGroqApiKey = cachedKey.trim().replace(/[\n\r\t\s]/g, "");
    
    // Seed system instruction if history is fresh
    if (chatHistoryArray.length === 0) {
        chatHistoryArray.push({
            role: "system",
            content: "You are HOUTS, an elite AI architecture workspace assistant. Help users structure clean digital web layouts. CRITICAL: Return ONLY valid HTML with embedded CSS/JS inside <style> and <script> tags. Do not explain the code, do not use markdown code blocks, do not include any conversational text."
        });
    }
    return true;
}
...
window.executeSecureChatPipeline = async function(userPromptText) {
    ...
    if (!activeGroqApiKey && !initializeLocalGroq()) {
        appendMessageRow("<strong>Workspace Setup Required:</strong> It looks like your Groq Cloud API Key isn't configured yet. Please open your <strong>Account Settings</strong> dashboard, head over to the system settings tab, and provide a secure Groq key to activate the layout assistant.", "ai");
    ...
    try {
        // Direct secure fetch request to Groq OpenAI-compatible endpoints
        const apiResponse = await fetch("https://api.groq.com/openai/v1/chat/completions", {
            method: "POST",
            headers: {
                "Authorization": `Bearer ${activeGroqApiKey}`,
                "Content-Type": "application/json"
            },
            body: JSON.stringify({
                model: "llama-3.3-70b-versatile", // Groq's top tier code/reasoning model
        ...
        if (rawContent) {
            ...
            appendMessageRow("The Groq engine responded successfully but returned an empty structural canvas. Please try adjusting your description query.", 'ai');
        }

    } catch (chatError) {
        console.error("Direct Groq framework issue:", chatError);
        if (typeof removeAiLoader === 'function') removeAiLoader(liveLoaderTrackingId);
        
        const rawErrorString = chatError.toString().toLowerCase();
        
        // Error intercepts tailored to Groq exceptions
        if (rawErrorString.includes("rate_limit") || rawErrorString.includes("quota") || rawErrorString.includes("429")) {
            appendMessageRow(`
                <div style="background: rgba(239, 68, 68, 0.05); border: 1px solid rgba(239, 68, 68, 0.2); padding: 1.25rem; border-radius: 12px; margin-top: 0.5rem; box-sizing: border-box;">
                    <p style="margin: 0 0 0.5rem 0; font-weight: 600; color: #ef4444; font-size: 1rem;">⚠️ Groq Service Rate Limit Active</p>
                    <p style="margin: 0; font-size: 0.9rem; color: var(--text-secondary); line-height: 1.5;">
                        You have made too many design requests in a short window. Groq's free tier allows rapid testing but has strict limits per minute. Please pause for 60 seconds and submit your layout description again.
                    </p>
                </div>
            `, 'ai');
            ...
            appendMessageRow(`
                <div style="background: rgba(239, 68, 68, 0.05); border: 1px solid rgba(239, 68, 68, 0.2); padding: 1.25rem; border-radius: 12px; margin-top: 0.5rem; box-sizing: border-box;">
                    <p style="margin: 0 0 0.5rem 0; font-weight: 600; color: #ef4444; font-size: 1rem;">🔑 Invalid Groq Credentials</p>
                    <p style="margin: 0; font-size: 0.9rem; color: var(--text-secondary); line-height: 1.5;">
                        The API key stored in your Account Settings was rejected by Groq. Make sure you generated it from the Groq Cloud Dashboard and didn't leave off any characters.
                    </p>
                </div>
            `, 'ai');
        } else {
            appendMessageRow(`
                <div style="background: rgba(244, 63, 94, 0.05); border: 1px solid rgba(244, 63, 94, 0.2); padding: 1.25rem; border-radius: 12px; margin-top: 0.5rem; box-sizing: border-box;">
                    <p style="margin: 0 0 0.5rem 0; font-weight: 600; color: #f43f5e; font-size: 1rem;">🌐 Network Endpoint Event</p>
                    <p style="margin: 0; font-size: 0.9rem; color: var(--text-secondary); line-height: 1.5;">
                        Unable to securely communicate with the Groq Cloud array: ${chatError.message || "Unknown routing flag"}. Check your connectivity and retry.
                    </p>
                </div>
            `, 'ai');
        }
    ...
    const showAuthModal = (mode, user = null) => {
        ...
        if (mode === 'signup' || mode === 'login') {
                    ...
                    <div class="modal-main-content">
                        ...
                        <div id="db-panel-settings" style="display: none; flex-direction: column; gap: 1.5rem;">
                            ...
                            <div style="background: rgba(255,255,255,0.02); border: 1px dashed rgba(255,255,255,0.1); padding: 1.25rem; border-radius: 12px; margin-bottom: 0.5rem;">
                                <label style="display: block; font-weight: 600; font-size: 0.95rem; margin-bottom: 0.25rem;">Local Groq Cloud API Key</label>
                                <p style="margin: 0 0 1rem 0; font-size: 0.8rem; color: #71717a;">Saved securely inside your localized device cache storage. Never travels outside your browser endpoint.</p>
                                <div style="display: flex; gap: 0.5rem;">
                                    <input type="password" id="local-api-key-input" placeholder="gsk_..." style="flex: 1; padding: 0.65rem 1rem; border-radius: 8px; border: 1px solid var(--border); background: var(--bg-surface); color: var(--text-primary); outline: none; font-size: 0.9rem;">
            ...
            tabSet.addEventListener('click', () => {
                ...
                const existingKey = localStorage.getItem('HOUTS_SECURE_GROQ_KEY');
                if (existingKey) {
                ...
                apiKeySaveBtn.addEventListener('click', () => {
                    ...
                    if(freshKeyValue) {
                        const sterileInput = freshKeyValue.replace(/[\n\r\t\s]/g, "");
                        localStorage.setItem('HOUTS_SECURE_GROQ_KEY', sterileInput);
                        
                        // Clear runtime context caches instantly
                        activeGroqApiKey = null;
                        chatHistoryArray = [];
                        initializeLocalGroq();
                        ...
                        localStorage.removeItem('HOUTS_SECURE_GROQ_KEY');
                        activeGroqApiKey = null;
                        chatHistoryArray = [];
