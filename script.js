...
window.startNewChat = function(e) {
    ...
    // Check for showToast function availability
    const toastCheck = document.createElement('div');
    if(document.querySelector('.houts-toast') === null) {
        // Fallback or silently proceed if toast isn't fully ready
...
window.executeSecureChatPipeline = async function(userPromptText) {
    ...
    // Store user message in context memory history cleanly
    ...
    try {
        // Inject structural prompt instructions into the system layer so it applies to the whole context correctly
        ...
        if (data && data.choices && data.choices[0] && data.choices[0].message) {
            ...
            // Store AI response to history to remember for subsequent chats
            ...
            if (auth.currentUser) {
                ...
                }, { merge: true }).catch(err => console.error("Database save exception:", err));
            }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Check if we need to load a chat via URL parameters on create.html
                ...
                db.collection('chats').where('userId', '==', user.uid).orderBy('updatedAt', 'desc').get().then(snapshot => {
                    ...
                    snapshot.forEach(doc => {
                        ...
                        btn.onclick = () => {
                            ...
                            // Check if already on the create layout, else redirect there
    ...
    auth.onAuthStateChanged((user) => {
        ...
        navMenuContainers.forEach(navMenuContainer => {
            ...
            if (user) {
                ...
                // Insert right before the logout button or at the end
