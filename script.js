...
window.executeSecureChatPipeline = async function(userPromptText) {
    ...
    try {
        ...
        if (rawContent) {
            ...
            if (typeof window.showToast === 'function') window.showToast("🎉 Architecture Canvas Compiled!", true);
        } else {
...
// Global Animation UI Engine Definitions
...
// Global Scope Pipeline Modal Managers
window.closeModalPipeline = function() {
    const overlay = document.getElementById('auth-modal-overlay');
    if (overlay) overlay.classList.remove('active');
};

window.showAuthModal = function(type, activeUser = null) {
    // Injects structural modal layer elements if missing from viewport nodes
    if (!document.getElementById('auth-modal-overlay')) {
        const overlayElement = document.createElement('div');
        overlayElement.id = 'auth-modal-overlay';
        overlayElement.className = 'modal-overlay'; 
        overlayElement.innerHTML = `<div class="settings-modal" id="modal-container-anchor"></div>`;
        document.body.appendChild(overlayElement);

        overlayElement.addEventListener('click', (e) => {
            if (e.target === overlayElement) window.closeModalPipeline();
        });
    }
    ...
    if (type === 'login' || type === 'signup') {
        container.innerHTML = `
            <button class="close-modal-btn" onclick="window.closeModalPipeline()">✕</button>
        ...
        document.getElementById('switch-to-signup')?.addEventListener('click', (e) => { e.preventDefault(); window.showAuthModal('signup'); });
        document.getElementById('switch-to-login')?.addEventListener('click', (e) => { e.preventDefault(); window.showAuthModal('login'); });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Injects dynamic notification helper styles if missing
    ...
    const setupAuthListeners = () => {
        document.querySelectorAll('.auth-link-login').forEach(el => {
            el.addEventListener('click', (e) => { e.preventDefault(); window.showAuthModal('login'); });
        });
        document.querySelectorAll('.nav-cta-btn').forEach(el => {
            if (el.id !== 'dashboard-action-trigger') {
                el.addEventListener('click', (e) => { e.preventDefault(); window.showAuthModal('signup'); });
            }
        });
    };
    setupAuthListeners();
    ...
    auth.onAuthStateChanged((user) => {
        ...
        if (user) {
            ...
            settingsBtn.addEventListener('click', (e) => { e.preventDefault(); window.showAuthModal('account', user); });
