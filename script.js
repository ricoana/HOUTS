...
window.saveWebsiteToAccount = function(encodedCode, btnElement) {
    try {
        ...
        if (currentUser) {
            ...
            .then(() => {
                if (typeof showToast === 'function') {
                    showToast('✨ Website saved to your account successfully!', true);
                }
            })
            .catch((err) => {
                console.error("Firestore save error:", err);
                if (typeof showToast === 'function') {
                    showToast('✨ Website saved locally to your profile!', true);
                }
            });
        } else {
            if (typeof showToast === 'function') {
                showToast('✨ Website saved to your local profile!', true);
            }
        }
    } catch (err) {
            ...
            const actionHtml = `
                <div style="background: rgba(99, 102, 241, 0.1); border: 1px solid #6366f1; padding: 1.25rem; border-radius: 12px; margin-top: 0.5rem; box-sizing: border-box; position: relative; z-index: 10;">
                    ...
                    <div style="display: flex; gap: 0.75rem; width: 100%; flex-wrap: wrap; position: relative; z-index: 20;">
                        ...
                        <button onclick="saveWebsiteToAccount('${encodedCode}', this)" style="flex: 1; min-width: 140px; height: 44px; line-height: 44px; text-align: center; background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.15); color: #ffffff; cursor: pointer; border-radius: 8px; font-weight: 600; font-size: 0.9rem; transition: background 0.2s, border-color 0.2s;">Save to Account</button>
                    </div>
                </div>
            `;
