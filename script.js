...
function initializeLocalGenAI() {
    ...
    // If key doesn't exist, reset instances gracefully and exit instead of erroring out
    ...
    try {
        aiInstance = new GoogleGenAI({ apiKey: cachedKey });
...
window.executeSecureChatPipeline = async function(userPromptText) {
    ...
    // Make sure we have a fresh layout initial run check or check if user dropped a key mid-session
