...
// --- MANUAL CHAT SAVING & SIDEBAR INTEGRATION ---
window.saveCurrentChat = async function() {
    const user = auth.currentUser;
    if (!user) {
        alert("You must be logged in to save your chat.");
        return;
    }
    if (!currentMessageHistory || currentMessageHistory.length === 0) {
        alert("There is no chat history to save yet.");
        return;
    }

    const firstUserMsg = currentMessageHistory.find(m => m.role === 'user');
    const chatTitle = firstUserMsg ? firstUserMsg.content.substring(0, 35) + (firstUserMsg.content.length > 35 ? '...' : '') : 'New Session';
    ...
    try {
        ...
        // If it's brand new, add createdAt timestamp
        await db.collection('chats').doc(currentChatId).set(chatPayload, { merge: true });
        ...
        // Dynamically prepend/update session in settings sidebar if container is open
        const historyList = document.getElementById('chat-history-list');
        if (historyList) {
            // Avoid duplicates if already present
            const existingBtn = historyList.querySelector(`[data-chat-id="${currentChatId}"]`);
            if (!existingBtn) {
                const btn = document.createElement('button');
                btn.className = 'chat-history-item';
                btn.setAttribute('data-chat-id', currentChatId);
                btn.innerHTML = `<div style="font-weight:600; font-size:0.95rem; margin-bottom:0.35rem; line-height:1.4;">${chatTitle}</div><div style="font-size:0.75rem; color:#a1a1aa;">Just now</div>`;
                btn.onclick = () => {
        ...
        if(saveBtn) saveBtn.innerHTML = "💾 Save Chat";
...
window.executeSecureChatPipeline = async function(userPromptText) {
    ...
    try {
        ...
        if (data && data.choices && data.choices[0] && data.choices[0].message) {
            ...
            // Reveal the explicit save button now that the website is successfully generated
            const saveBtn = document.getElementById('save-chat-btn');
            if (saveBtn) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (urlChatId) {
        auth.onAuthStateChanged(user => {
            if(user) {
                db.collection('chats').doc(urlChatId).get().then(doc => {
                    if(doc.exists && doc.data().userId === user.uid) {
                        currentChatId = doc.id;
                        currentMessageHistory = doc.data().messages || [];
    ...
    const injectModalHTML = () => {
        ...
        styleTag.innerHTML = `
            ...
            :root.light-mode .chat-history-item { 
                background: #ffffff; border: 1px solid rgba(0,0,0,0.08); color: #0a0a0c; 
            }
            :root.light-mode .chat-history-item:hover { background: #f4f4f5; }
    ...
    const showAuthModal = (mode, user = null) => {
        ...
        if (mode === 'signup' || mode === 'login') {
                ...
                <div class="modal-dashboard-layout">
                    ...
                    <div class="modal-sidebar">
                        <div style="display: flex; flex-direction: column; gap: 0.5rem; width: 100%; flex-direction: inherit;">
                            ...
                            <button id="db-tab-chats" class="modal-sidebar-btn">💬 Chats</button>
                    ...
                    <div class="modal-main-content">
                        ...
                        <div id="db-panel-chats" style="display: none; flex-direction: column; gap: 1.5rem;">
                            <div>
                                <h1 style="margin: 0 0 0.25rem 0; font-size: 1.75rem; font-weight: 700; letter-spacing:-0.5px;">Chat History</h1>
                                <p style="margin: 0; font-size: 0.9rem; color: #a1a1aa;">Resume your previous architecture sessions.</p>
                            </div>
                            <hr style="border: 0; border-top: 1px solid rgba(255,255,255,0.06); margin: 0;">
                            <div id="chat-history-list" style="display: flex; flex-direction: column; gap: 0.75rem; overflow-y: auto;">
            ...
            const tabChats = document.getElementById('db-tab-chats');
            const tabSet = document.getElementById('db-tab-settings');
            const panelProf = document.getElementById('db-panel-profile');
            const panelChats = document.getElementById('db-panel-chats');
            const panelSet = document.getElementById('db-panel-settings');

            tabProf.addEventListener('click', () => {
                tabProf.classList.add('active'); tabSet.classList.remove('active'); tabChats.classList.remove('active');
                panelProf.style.display = 'flex'; panelSet.style.display = 'none'; panelChats.style.display = 'none';
            });
            
            tabChats.addEventListener('click', () => {
                tabChats.classList.add('active'); tabProf.classList.remove('active'); tabSet.classList.remove('active');
                panelChats.style.display = 'flex'; panelProf.style.display = 'none'; panelSet.style.display = 'none';
                
                const listContainer = document.getElementById('chat-history-list');
                listContainer.innerHTML = '<span style="color:#a1a1aa; font-size:0.9rem;">Fetching sessions...</span>';
                
                db.collection('chats').where('userId', '==', user.uid).orderBy('updatedAt', 'desc').get().then(snapshot => {
                    listContainer.innerHTML = '';
                    if(snapshot.empty) {
                        listContainer.innerHTML = '<p style="color:#a1a1aa; font-size:0.9rem;">No past sessions found.</p>';
                        return;
                    }
                    snapshot.forEach(doc => {
                        const data = doc.data();
                        const firstMsg = (data.messages || []).find(m => m.role === 'user');
                        const title = firstMsg ? firstMsg.content.substring(0, 50) + (firstMsg.content.length > 50 ? '...' : '') : 'Empty Session';
                        const dateString = data.updatedAt ? data.updatedAt.toDate().toLocaleString() : 'Just now';

                        const btn = document.createElement('button');
                        btn.className = 'chat-history-item';
                        btn.setAttribute('data-chat-id', doc.id);
                        btn.innerHTML = `<div style="font-weight:600; font-size:0.95rem; margin-bottom:0.35rem; line-height:1.4;">${title}</div><div style="font-size:0.75rem; color:#a1a1aa;">${dateString}</div>`;
                        btn.onclick = () => {
                            currentChatId = doc.id;
                            currentMessageHistory = data.messages || [];
                            hideAuthModal();
                            ...
                            if (window.location.pathname.includes('create.html')) {
                                ...
                                window.location.href = './create.html?chat=' + doc.id;
                            }
                        };
                    ...
                    console.error(err);
                    listContainer.innerHTML = '<p style="color:#ef4444; font-size:0.9rem;">Failed to load history.</p>';
                });
            });
            
            tabSet.addEventListener('click', () => {
                tabSet.classList.add('active'); tabProf.classList.remove('active'); tabChats.classList.remove('active');
                panelProf.style.display = 'none'; panelChats.style.display = 'none'; panelSet.style.display = 'flex';
    ...
    auth.onAuthStateChanged((user) => {
        ...
        const newChatBtn = document.getElementById('new-chat-header-btn');
