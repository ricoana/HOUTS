...
window.saveWebsiteToAccount = function(encodedCode) {
    try {
        ...
        if (currentUser) {
            db.collection('users').doc(currentUser.uid).collection('saved_websites').add(projectEntry)
            .then(() => {
            ...
            .catch((err) => {
                ...
                saveToLocalFallback(projectEntry);
            });
        } else {
            saveToLocalFallback(projectEntry);
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const showAuthModal = (mode, user = null) => {
        ...
        if (mode === 'signup' || mode === 'login') {
            ...
            tabWeb.addEventListener('click', () => {
                ...
                // Fetch from Firestore saved_websites subcollection
                db.collection('users').doc(user.uid).collection('saved_websites').orderBy('createdAt', 'desc').get()
                .then((snapshot) => {
                    let projects = [];
                    ...
                    // Fallback to local storage if empty or merge
    ...
    function renderSavedWebsites(projects, containerEl) {
        ...
        projects.forEach(proj => {
            ...
            card.style.cssText = "background: rgba(255,255,255,0.03); border: 1px solid var(--border); padding: 1.15rem; border-radius: 12px; display: flex; justify-content: space-between; align-items: center; gap: 1rem; flex-wrap: wrap;";
            card.innerHTML = `
                <div>
                    <h4 style="margin: 0 0 0.25rem 0; font-size: 1rem; font-weight: 600; color: var(--text-primary);">${proj.title || 'Generated Layout'}</h4>
                    <p style="margin: 0; font-size: 0.75rem; color: var(--text-secondary);">${dateStr}</p>
                </div>
                <div style="display: flex; gap: 0.5rem;">
